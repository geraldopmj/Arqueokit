...
class FunaiDownloader(QgsProcessingAlgorithm):
    ...
    def _check_service_available(url, timeout_ms=5000):
        ...
        if error_code != 0:
            raise QgsProcessingException(f"Servidor da FUNAI indisponível. Detalhes: {reply.errorString()}")

        status_code = enum_value(reply.attribute(http_status_attr)) if http_status_attr is not None else None
        if status_code and int(status_code) >= 400:
            raise QgsProcessingException(f"Servidor da FUNAI respondeu com status {status_code}.")
